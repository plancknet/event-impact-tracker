...
serve(async (req) => {
  ...
  try {
    ...
    const planType: PlanType = body?.planType === "INFLUENCER" ? "INFLUENCER" : "STANDARD";

    const standardPriceId = Deno.env.get("STRIPE_STANDARD_PRICE_ID") ?? "";
    const influencerPriceId = Deno.env.get("STRIPE_INFLUENCER_PRICE_ID") ?? "";

    if (!standardPriceId || !influencerPriceId) {
      throw new Error("Stripe price IDs nao configurados.");
    }
